...
- `TalentManager` (`/talent`): class and profession point pools from levels (`talents.*` config); WoW-style talent gating via optional `requires-job`/`requires-spec`/`requires-profession` definition keys (gated talents stop contributing and refund on respec); attribute effects applied as player attribute modifiers (idempotent re-apply on join via `TalentAttributeListener`), XP-bonus effects queried by the XP listeners.
...
- `ProfessionManager` (PDC keys `profession_gathering`, `profession_crafting`, `profession_xp_<id>`) implements WoW-style professions: one GATHERING (miner/herbalist/lumberjack) + one CRAFTING (armorer/alchemist/enchanter) primary slot per player, SECONDARY professions (fisherman/cook) active for everyone; per-profession XP survives slot changes; progressive level curve (`professions.leveling`). `ProfessionXpListener` covers 8 activity sources; `/spec respec <class|profession>` drops a spec for faction currency (`specializations.respec-cost`).
