...
- Second CI run [34045537799](https://github.com/MilCsik09/IceSMP/actions/runs/34045537799) exposed a pre-existing config-audit mismatch: commit `3013ba9c246f3900911a9f0a3251a2430bfc24cb` had added `prologue`, `blood-moon` and `season-finale` to the major-event gate. The audit now accepts only that exact ordered before/after list; all other leaf checks remain. The local authority audit passes with 24,640 leaves.
- The same run found corrupted committed `dev-assets/trash/source/phase-batch-001.png` and `phase-batch-002.png`, introduced by `82a981121706d51f7678516e2956475974720f71`. Re-encoding their bytes to base64 reveals embedded `Warningtruncatedoutputoriginaltokencount` text. The first file also has a damaged IDAT payload; stripping its prefix cannot repair it. These are not LFS pointers. The source-sprite gate remains failing; existing rendered textures do not establish reproducibility of these missing source bytes. No source/texture has been replaced and no validation is skipped. The workflow uses `--continue` to collect independent regression evidence while preserving the failed build exit status.
