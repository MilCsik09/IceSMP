...
- the remaining 5 classes and 14 specializations have not passed this vertical-slice gameplay gate;
...
- the physical world content for `warrior_berserker_broken_horn` (Törött Kürt), `warrior_guardian_last_wall` (Utolsó Fal), `evoker_devastation_trial`, `evoker_preservation_trial`, `archer_sharpshooter_trial`, `archer_beast_master_trial`, `shaman_elemental_trial`, `shaman_enhancement_trial`, `shaman_tidal_trial`, `monk_windwalker_trial`, `monk_brewmaster_trial`, `monk_mistweaver_trial`, `paladin_holy_trial`, `paladin_retribution_trial`, `paladin_protection_trial`, `demon_hunter_havoc_trial`, `demon_hunter_vengeance_trial`, `druid_feral_trial`, `druid_lunar_trial`, `druid_ironbark_trial` and `druid_restoration_trial` is a builder/event gate; no fabricated coordinates or fake arena completion is part of the code slices. The Evoker, Archer, Shaman, Monk, Paladin, Demon Hunter and Druid trial ids intentionally carry no lore names because the canonical game-design document (`IceSMP_Kasztok_es_Specializaciok_Teljes_Jatekdesign_VEGLEGES.md`) is not available in this repository or the session file library; the Evoker, Archer, Shaman, Monk, Paladin, Demon Hunter and Druid doctrine identifiers are likewise mechanic-descriptive working names pending canonical verification;
