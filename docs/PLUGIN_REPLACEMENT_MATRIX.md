...
| AxAFKZone / AxAPI | nincs termékigény jutalmazó AFK-zónára | globális AFK, `/afk`, tablista és meglévő lifecycle | semmi — scope törölve | reward-zóna, selection, migráció | **CANCELLED / OUT OF SCOPE** | globális AFK regresszió/playtest |
| GSit | `/sit`, `/sit fel`, click-to-sit, seat lifecycle, blokkforma-, világ- és materialpolicy | natív sit-only manager, listener, config és PDC seat entity | valódi Folia átvételi teszt | lay, crawl, player/NPC sitting, stacking, teljes GSit API | **READY** kódszinten, runtime feltételes | `sitRegressionTest`, teljes build; manuális runtime még kell |
...
| CrazyCrates | preview/lista, permission/world/cooldown/key-count/stats, strict rewards | fizikai crate, PDC-kulcs, weighted reward, GUI/reveal, admin set/give/list | külön crate scope | legacy key/config/location, minden upstream animáció | **PARTIAL** | crate scope-ban |
