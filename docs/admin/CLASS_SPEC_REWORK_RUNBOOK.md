...
- **Harmónia és Évszak (másodlagos mechanika):** nature casts build Harmónia with lazy decay. The separate primary class resource remains **Természeti Erő**. An **alakváltás on the existing form system** releases the whole Harmónia pool at once as the season bound to that form — Tavasz = Holdforma (heal + regeneration), Nyár = Párducforma (strength), Ősz = Utazóforma (a short window that empowers the next casts through the capped shared power pipeline), Tél = Medveforma (resistance + absorption). Below `harmony.release-threshold` the form still works, only the blessing is withheld.
