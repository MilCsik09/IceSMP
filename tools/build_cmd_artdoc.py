...
def main():
    ...
    out.append('- **Jelenlegi állapot:** MINDEN mostani textúra ideiglenes (generált placeholder, illetve az 5 pénz-item egy PRÓBA-lapról bevágva) — a végleges készletet a textúra-készítő adja; semmi sincs kőbe vésve.')
    ...
    out.append('## Stílus-szabályok (vanilla-konzisztencia)')
    out.append('')
    out.append('1. **Egységes felbontás:** a felbontást a textúra-készítő választja meg (vanilla-hű 16×16 vagy részletesebb 32×32) — de a TELJES pack egységesen ugyanazt használja, vegyes felbontás tilos. Az import-szkript bármelyikre méretez (`--size`).')
    out.append('2. **Margó:** a tárgy ne érjen a vászon széléig — a vászon ~80%-át töltse ki, középre igazítva (16-osnál ~1-2 px, 32-esnél ~3-4 px üres perem).')
    out.append('3. **Sziluett-olvashatóság:** az item egy vanilla tárgy helyén jelenik meg — első ránézésre ugyanannak a tárgy-osztálynak tűnjön (bot=bot, sisak=sisak). Kard/szerszám 45°-ban átlósan: markolat balra-le, hegy jobbra-fel.')
    out.append('4. **Kontúr:** 1 px sötét körvonal a külső élen, de NEM tiszta fekete — az anyagszín legmélyebb árnyalata; a belső vonalak még lágyabbak.')
    out.append('5. **Paletta-fegyelem:** anyagonként 4-8 tónus, kemény pixel-átmenetek — semmi blur, anti-aliasing vagy színátmenet; dither csak nagyon indokoltan.')
    out.append('6. **Fényirány:** mindig bal-felső fényforrás (világos él fent/balra, mély tónus lent/jobbra); vetett árnyék a sziluetten kívül nincs.')
    out.append('7. **Alfa igen/nem:** minden pixel vagy teljesen fedő, vagy teljesen átlátszó — félig átlátszó élpixel TILOS (a játékban csúnya szegély lesz belőle).')
    out.append('8. **Telítettség:** a vanilla visszafogott, földes paletta mellett a neon kiabál — izzó akcent (lich-türkiz, láng) csak kevés pixelen (2-6 fénypont).')
    out.append('9. **Kicsiben is működjön:** az inventoryban ~16-32 képernyő-pixel látszik — a 2 px-nél kisebb részlet eltűnik; minden darabot kicsinyítve is ellenőrizz.')
    out.append('10. **Perspektíva:** lapos sprite szemből (vagy 45°-os szerszám) — nem izometrikus/3D nézet.')
    out.append('')
    ...
    out.append('')
    out.append('## AI-generálási prompt-sablon')
    out.append('')
    out.append('Ha a textúrákat képgenerátorral készíted (a próba-érme lap is így készült), ez a')
    out.append('sablon jó kiindulás. **Tippek:** egyszerre csak 4-6 itemet kérj egy lapra (úgy')
    out.append('tartja a stílust); sima FEHÉR hátteret kérj (az importáló leszedi); NE kérj')
    out.append('szöveget a képre; az itemek leírását a fenti lista **Ábrázolás + Színvilág +')
    out.append('Hangulat** soraiból másold be. A kész lapot az import-szkript vágja be a packba.')
    out.append('')
    out.append('```')
    out.append('Pixel art sprite sheet of [N] fantasy RPG item icons for a Minecraft-style game,')
    out.append('arranged in a single row on a plain white background, with clear spacing between items.')
    out.append('')
    out.append('Items:')
    out.append('1. [első item: Ábrázolás + Színvilág sora, angolra fordítva vagy magyarul]')
    out.append('2. [második item…]')
    out.append('...')
    out.append('')
    out.append('Style: chunky 16-bit pixel art, limited palette (4-8 tones per material),')
    out.append('1px dark outline in the material\'s own darkest shade (not pure black),')
    out.append('light source from the top-left, crisp hard pixels, no anti-aliasing,')
    out.append('no gradients, no drop shadows, flat 2D sprite view (no isometric perspective),')
    out.append('each item centered with a small margin, consistent style across all icons,')
    out.append('cohesive fantasy RPG game asset set, high quality pixel art.')
    out.append('```')
    out.append('')
    out.append('**Kitöltött példa (a pénz-lap):**')
    out.append('')
    out.append('```')
    out.append('Pixel art sprite sheet of 5 fantasy RPG item icons for a Minecraft-style game,')
    out.append('arranged in a single row on a plain white background, with clear spacing between items.')
    out.append('')
    out.append('Items:')
    out.append('1. A round gold coin with a ridged rim and an embossed FLAME emblem in the center, warm gold with orange accents.')
    out.append('2. A round silver coin with a ridged rim and an embossed SNOWFLAKE emblem, cool silver with icy light-blue accents.')
    out.append('3. A round gold coin with an embossed MERCHANT SCALES emblem, warm gold with amber accents.')
    out.append('4. A worn dark-steel coin with an embossed SKULL emblem, tiny cold TURQUOISE glow in the skull\'s eye sockets, chipped edges.')
    out.append('5. A worn leather coin pouch tied with a drawstring, 2-3 gold coins peeking out at the neck, warm leather brown.')
    out.append('')
    out.append('Style: chunky 16-bit pixel art, limited palette (4-8 tones per material),')
    out.append('1px dark outline in the material\'s own darkest shade (not pure black),')
    out.append('light source from the top-left, crisp hard pixels, no anti-aliasing,')
    ...
    out.append('```')
    out.append('')
    out.append('A frakció-akcentek angol fordítása a prompthoz: RED=„glowing ember orange-red”,')
    out.append('BLUE=„icy light blue and silver”, NEUTRAL=„merchant gold and amber”,')
    out.append('DARK=„bone white, pitch black, with cold turquoise lich-glow accents”.')
    out.append('')
