...
- Célzott frakciótesztek: `./gradlew factionPassiveRegressionTest factionPassiveHardeningRegressionTest factionReworkRegressionTest factionTreasuryRegressionTest`; a normál
  `check` részei, de frakció-reworknél külön is futtatandók.
...
- **Config és content authority**: operator-hangolás `src/main/resources/config/*.yml` + schema-bounded gyökér `config.yml`; kézzel authorolt gameplay `src/main/resources/content/**`; lokalizáció `src/main/resources/messages/**`. A `ConfigManager` explicit operator/content allowlistet tölt. Egy gameplay-definíciónak egyetlen olvasható, verziózott `content/**` YAML/JSON gazdája lehet; generált gameplay overlay és konkurens Java-katalógus tilos. Generátor csak validálhat, evidence/reportot vagy resource-pack artifactot építhet, gameplayt nem authorolhat. A canonical contentet közvetlenül szerkeszd, majd releváns audit/regresszió + `scripts/check_consistency.py` kötelező.
- **Élő-config konvenció**: operator-kulcsot híváskor olvass (`configManager.getX` a use-site-on, ne konstruktorban). Ingame felülbírálás: `/icesmp config get|set|unset|list|find|menu` + kattintható GUI (`ConfigMenuGUI`, jog: `icesmp.admin.config`) — az írás EGYETLEN útja a szerializált `ConfigManager.applyOverride`. A merged YAML és az override-pathok egyetlen immutable `ConfigManager.ConfigSnapshot` objektumban, atomikus referenciacserével publikálódnak. Operator reload fail-safe és schema-bounded; canonical content változtatás restart-required. Boolean-kulcsok: `allow-<szabály>` (true=SZABAD), legacy invertált kulcs csak migration fallback.
...
- **Frakciótagság és passzívok:** assignment nélkül a játékos Menedék-vendég, nem implicit `NEUTRAL`; gameplay-jogosultsághoz mindig `FactionMembership`/`FactionManager.isEligibleForFactionBenefits` kell, a `getEconomyFaction` csak megjelenítési/valuta-fallback. Tagságot csak a durable assignment+history candidate sikeres mentése után publikálj; fizetős váltásnál `FactionSwitchJournal` kötelező. Infamy, Wanted, Exile, DARK Oath és Membership külön PlayerProfile-tengely; DARK-belépés: Exile → `/faction status eskü` → megerősített join. A sebzés-, exhaustion- és targetdöntést a tiszta `FactionPassivePolicy` hozza, a `FactionPassiveListener` csak Paper/Folia adapter; CANCEL döntésnél a targetet ténylegesen `null`-ra állítja. A DARK retaliation játékos–mob páronkénti, a nearby alert minden riasztott mobnak külön lease. Vérhold az ambient és vad DARK truce fölött áll. DARK normál gyógyítása fix `0.70`, kivéve Vérhold/DUNGEON; polgári bolt és karaván tiltott, a feketepiac kivétel, a kompár `2×`. Nincs aktív adó-, food-duty- vagy Whisper-decay scheduler és nincs új frakció-HUD-mérő. A Suttogó tanú–cél bizonyíték egyszer használható, három vád fixen leleplez. Signature-food buff csak fogyasztáskori live tagsággal futhat; itembe égetett feltétel nélküli frakcióbuff tilos.
...
- **Spellek**: deklaratív `ConfiguredSpell.builder(...)` a `spells/SpellCatalog`-ban + unlock a `content/progression/classes.yml`-ben; külön osztály csak valóban állapotos spellnek (kötelező `clearPlayerState` override).
- **Dependency contract**: FancyNpcs kötelező production gameplay dependency és a canonical TALK_TO_NPC/onboarding híd; hiányzó vagy inkompatibilis lockolt verziónál a startup fail-closed. PlaceholderAPI, LibsDisguises, WorldGuard/WorldEdit és LuckPerms opcionális jelenlegi integráció. MythicMobs nem tervezett; PacketEvents és FancyDialogs csak jövőbeli jelölt, nem runtime-dependency. Build-oldalon csak PlaceholderAPI és LibsDisguises `compileOnly`; a többi jelenlegi híd reflexiós. A first-party HUD kizárólag immutable `HudSnapshot` adatot olvas, és nem gameplay authority.
