...
/**
 * Dependency-free Profile v2 gateway regressions for the Halállovag rollout: the allowlist admits
 * death_knight, the DARK Szentségtelen still answers to the existing gate system, and doctrines stay
 * slot-local.
 */
public final class DeathKnightProfileRegressionSuite {

    private static final UUID PLAYER = UUID.fromString("00000000-0000-0000-0000-0000000004a1");
    private static int assertions;

    private DeathKnightProfileRegressionSuite() {
    }

    public static void main(final String[] args) {
        deathKnightSecondSlotUnlocksAndSwitches();
        darkUnholyObeysTheExistingGates();
        deathKnightDoctrineMasteryAndCapstoneStaySlotLocal();
        System.out.println("Death Knight profile regression suite passed. assertions=" + assertions);
    }

    private static void deathKnightSecondSlotUnlocksAndSwitches() {
        final Harness h = harness(ClassSpecSection.builder()
                .primaryClassId("death_knight").classLevel(1).build());
        final var xp = h.gateway.mutateClassExperience(PLAYER,
                new ClassSpecProfileGateway.ClassExperienceRequest(
                        ...
                        100_000, 100, 0, 28, "deathknight-xp-28")).toCompletableFuture().join();
        check(xp.committed(), "class XP mutation commits for death_knight");
        check(h.store.profile.secondSpecUnlocked(),
                "death_knight second spec unlocks through the allowlist");

        check(h.gateway.select(PLAYER, new ClassSpecProfileGateway.SelectRequest(
                        "blood", LoadoutSlot.FIRST, openGates()))
                .toCompletableFuture().join().committed(), "death_knight learns Vérlovag");
        check(h.gateway.select(PLAYER, new ClassSpecProfileGateway.SelectRequest(
                        "frost", LoadoutSlot.SECOND, openGates()))
                .toCompletableFuture().join().committed(), "death_knight learns Fagylovag second");
        final var switched = h.gateway.switchLoadout(PLAYER,
                        new ClassSpecProfileGateway.SwitchRequest(LoadoutSlot.SECOND))
                .toCompletableFuture().join();
        check(switched.committed(), "death_knight loadout switching is enabled");
        check(h.gateway.activeSpecId(PLAYER).orElseThrow().equals("frost"),
                "switch activates the Fagylovag loadout");
    }

    private static void darkUnholyObeysTheExistingGates() {
        final Harness h = harness(ClassSpecSection.builder()
                .primaryClassId("death_knight").classLevel(30).classExperience(100_000)
                .secondSpecUnlocked(true).build());
        final var blocked = h.gateway.select(PLAYER, new ClassSpecProfileGateway.SelectRequest(
                "unholy", LoadoutSlot.FIRST, closedGates())).toCompletableFuture().join();
        check(blocked.status() == ProfileMutationResult.Status.REJECTED,
                "the DARK Szentségtelen is refused while its gates are unsatisfied");
        ...
        check(h.gateway.select(PLAYER, new ClassSpecProfileGateway.SelectRequest(
                        "unholy", LoadoutSlot.FIRST, satisfiedGates()))
                .toCompletableFuture().join().committed(),
                "satisfied gates admit the Szentségtelen through the existing DARK system");
        check(h.store.profile.loadout(LoadoutSlot.FIRST).specializationId().equals("unholy"),
                "the admitted DARK spec occupies its loadout");
    }

    private static void deathKnightDoctrineMasteryAndCapstoneStaySlotLocal() {
        final ClassLoadout blood = loadout("blood", LoadoutStatus.ACTIVE)
                .withDoctrineChoice("level_30", "suru_ver")
                .withDoctrineChoice("level_40", "melyebb_rovas")
                .withMastery(new MasteryProgress(5, 530))
                .withCapstoneStatus(CapstoneStatus.COMPLETED);
        final ClassLoadout frost = loadout("frost", LoadoutStatus.INACTIVE)
                .withDoctrineChoice("level_30", "dermeszto_kez")
        ...
        final ClassSpecSection profile = ClassSpecSection.builder()
                .revision(5).primaryClassId("death_knight").classLevel(50).classExperience(999_999)
                .secondSpecUnlocked(true)
                .loadout(LoadoutSlot.FIRST, blood)
                .loadout(LoadoutSlot.SECOND, frost)
                .activeSlot(LoadoutSlot.FIRST)
                .build();
        check(profile.loadout(LoadoutSlot.FIRST).doctrineChoices()
                        .equals(Map.of("level_30", "suru_ver", "level_40", "melyebb_rovas")),
                "Vérlovag doctrines stay in their own slot");
        check(profile.loadout(LoadoutSlot.SECOND).doctrineChoices()
                        .equals(Map.of("level_30", "dermeszto_kez")),
                "Fagylovag doctrines stay in their own slot");
        ...
        expectFailure(() -> profile.loadout(LoadoutSlot.FIRST)
                        .withDoctrineChoice("level_30", "hosszu_emlekezet"),
