...
public final class ClassSpecApplicationRegressionSuite {
    ...
    private static void atomicSessionCallbackFence(){
        ProfileSessionRegistry sessions=new ProfileSessionRegistry();UUID old=sessions.begin(PLAYER);sessions.markReady(PLAYER,old);CountDownLatch entered=new CountDownLatch(1),release=new CountDownLatch(1),beginStarted=new CountDownLatch(1);AtomicInteger effects=new AtomicInteger();CompletableFuture<Boolean> oldCallback=CompletableFuture.supplyAsync(()->sessions.runIfCurrent(PLAYER,old,()->{entered.countDown();try{release.await();}catch(InterruptedException x){Thread.currentThread().interrupt();throw new CompletionException(x);}effects.incrementAndGet();}));await(entered);CompletableFuture<UUID> replacement=CompletableFuture.supplyAsync(()->{beginStarted.countDown();return sessions.begin(PLAYER);});await(beginStarted);expect(TimeoutException.class,()->replacement.get(50,TimeUnit.MILLISECONDS));release.countDown();check(oldCallback.join(),"current callback admitted");UUID next=replacement.join();check(effects.get()==1,"current callback ran exactly once");check(!sessions.runIfCurrent(PLAYER,old,effects::incrementAndGet),"retired callback rejected");check(sessions.isCurrent(PLAYER,next)&&effects.get()==1,"retired callback cannot affect replacement session");
    }
    ...
    private static void companionDeltaSerializationAndReceipts(){
        UUID id=UUID.fromString("00000000-0000-0000-0000-000000000398");
        ControlledStore store=new ControlledStore(activeWithCompanion("beast_master",id,1,10));
        ProfileSessionRegistry sessions=readySessions();DefaultClassSpecProfileGateway gateway=new DefaultClassSpecProfileGateway(store,ClassSpecRuntimePort.noop(),sessions);
        var a=gateway.mutateCompanionProgress(PLAYER,new ClassSpecProfileGateway.CompanionProgressRequest(LoadoutSlot.FIRST,id,2,100,0,30,"pet-delta-a")).toCompletableFuture();
        var b=gateway.mutateCompanionProgress(PLAYER,new ClassSpecProfileGateway.CompanionProgressRequest(LoadoutSlot.FIRST,id,2,100,0,30,"pet-delta-b")).toCompletableFuture();
        check(store.saveCalls==1,"second companion XP mutation queued");store.commitPending();a.join();store.awaitSaveCalls(2);store.commitPending();b.join();
        CompanionProfile accumulated=store.profile.loadout(LoadoutSlot.FIRST).companionRoster().get(id);check(accumulated.experience()==14,"concurrent +2/+2 accumulates to 14");check(accumulated.level()==1,"delta accumulation keeps level");

        Harness levels=harness(activeWithCompanion("beast_master",id,1,8),ClassSpecRuntimePort.noop());
        var multi=levels.gateway.mutateCompanionProgress(PLAYER,new ClassSpecProfileGateway.CompanionProgressRequest(LoadoutSlot.FIRST,id,20,10,5,30,"pet-multi")).toCompletableFuture().join();
        check(multi.committed(),"multi-level companion delta committed");CompanionProfile multiPet=levels.store.profile.loadout(LoadoutSlot.FIRST).companionRoster().get(id);check(multiPet.level()==3&&multiPet.experience()==3,"multi-level curve uses latest durable progress");
        long replayRevision=levels.store.profile.revision();check(levels.gateway.mutateCompanionProgress(PLAYER,new ClassSpecProfileGateway.CompanionProgressRequest(LoadoutSlot.FIRST,id,20,10,5,30,"pet-multi")).toCompletableFuture().join().status()==ProfileMutationResult.Status.NO_CHANGE,"same progress receipt replays idempotently");check(levels.store.profile.revision()==replayRevision,"progress replay does not revise profile");check(levels.gateway.mutateCompanionProgress(PLAYER,new ClassSpecProfileGateway.CompanionProgressRequest(LoadoutSlot.FIRST,id,21,10,5,30,"pet-multi")).toCompletableFuture().join().status()==ProfileMutationResult.Status.REJECTED,"progress receipt binds delta payload");

        Harness capped=harness(activeWithCompanion("beast_master",id,2,14),ClassSpecRuntimePort.noop());
        check(capped.gateway.mutateCompanionProgress(PLAYER,new ClassSpecProfileGateway.CompanionProgressRequest(LoadoutSlot.FIRST,id,100,10,5,3,"pet-cap")).toCompletableFuture().join().committed(),"max-level progress commits");CompanionProfile maxed=capped.store.profile.loadout(LoadoutSlot.FIRST).companionRoster().get(id);check(maxed.level()==3,"max-level cap enforced");check(maxed.experience()==19,"max-level XP remains bounded below next threshold");

        Harness rename=harness(activeWithCompanion("beast_master",id,1,0),ClassSpecRuntimePort.noop());
        var renameRequest=new ClassSpecProfileGateway.CompanionMutationRequest(LoadoutSlot.FIRST,ClassSpecProfileGateway.CompanionMutationRequest.Kind.RENAME,id,null,"Fang",0,0,0,List.of(),Map.of(),"pet-rename-bound");check(rename.gateway.mutateCompanion(PLAYER,renameRequest).toCompletableFuture().join().committed(),"rename receipt created");check(rename.gateway.mutateCompanion(PLAYER,renameRequest).toCompletableFuture().join().status()==ProfileMutationResult.Status.NO_CHANGE,"same rename payload replays");var changedRename=new ClassSpecProfileGateway.CompanionMutationRequest(LoadoutSlot.FIRST,ClassSpecProfileGateway.CompanionMutationRequest.Kind.RENAME,id,null,"Claw",0,0,0,List.of(),Map.of(),"pet-rename-bound");check(rename.gateway.mutateCompanion(PLAYER,changedRename).toCompletableFuture().join().status()==ProfileMutationResult.Status.REJECTED,"rename receipt binds text");

        Harness add=harness(active("beast_master",Map.of()),ClassSpecRuntimePort.noop());CompanionProfile first=new CompanionProfile(id,"beast_master.stable","WOLF","Fang",1,0,"","ACTIVE",List.of(),0,Map.of("bond","one"));var addRequest=new ClassSpecProfileGateway.CompanionMutationRequest(LoadoutSlot.FIRST,ClassSpecProfileGateway.CompanionMutationRequest.Kind.ADD,id,first,"",0,0,0,List.of(),Map.of(),3,"pet-add-bound");check(add.gateway.mutateCompanion(PLAYER,addRequest).toCompletableFuture().join().committed(),"ADD receipt created");check(add.gateway.mutateCompanion(PLAYER,addRequest).toCompletableFuture().join().status()==ProfileMutationResult.Status.NO_CHANGE,"same ADD payload replays");CompanionProfile changed=first.withPersistentState(Map.of("bond","two"));var changedAdd=new ClassSpecProfileGateway.CompanionMutationRequest(LoadoutSlot.FIRST,ClassSpecProfileGateway.CompanionMutationRequest.Kind.ADD,id,changed,"",0,0,0,List.of(),Map.of(),3,"pet-add-bound");check(add.gateway.mutateCompanion(PLAYER,changedAdd).toCompletableFuture().join().status()==ProfileMutationResult.Status.REJECTED,"ADD receipt binds canonical companion payload");
    }
    ...
    private static ClassSpecSection activeWithCompanion(String spec,UUID id,int level,long xp){String clazz=ClassSpecCatalog.parentOf(spec);String namespace=ClassSpecCatalog.companionNamespace(spec);CompanionProfile companion=new CompanionProfile(id,namespace,"WOLF","Fang",level,xp,"","ACTIVE",List.of(),0,Map.of());return ClassSpecSection.builder().primaryClassId(clazz).classLevel(25).activeSlot(LoadoutSlot.FIRST).loadout(LoadoutSlot.FIRST,new ClassLoadout(spec,LoadoutStatus.ACTIVE,null,Map.of(),MasteryProgress.empty(),null,Set.of(),"",CapstoneStatus.LOCKED,Map.of(id,companion),Map.of("companion.active_id",id.toString()),"")).build();}
    private static GateSnapshot satisfied(){return new GateSnapshot(GateState.satisfied(),Map.of(GateState.Gate.FACTION,"dark",GateState.Gate.OATH,"sinner",GateState.Gate.QUEST,"quest"));}
