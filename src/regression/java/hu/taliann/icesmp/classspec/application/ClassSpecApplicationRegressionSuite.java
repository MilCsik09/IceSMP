...
public final class ClassSpecApplicationRegressionSuite {
    ...
    private static void greenfieldClassAndSpec(){
        Harness h=harness(ClassProfile.empty(PLAYER,0),ClassSpecRuntimePort.noop());
    ...
    private static void activationReconcileBeforeReady(){
        ...
        sessions.markReady(PLAYER,token);check(gateway.isSessionReady(PLAYER),"session becomes ready after activation reconcile");
    }
    ...
    private static void runtimeFailureIsVisible(){
        ClassSpecRuntimePort failing=new ClassSpecRuntimePort(){public CompletionStage<Void> profileCommitted(UUID p,UUID t,ClassProfile a,ClassProfile b,MutationKind k){return CompletableFuture.failedFuture(new IllegalStateException("runtime boom"));}public CompletionStage<Void> failClosed(UUID p,UUID t,String r){return CompletableFuture.completedFuture(null);}};
        Harness h=harness(ClassProfile.empty(PLAYER,0),failing);var result=h.gateway.assignClass(PLAYER,new ClassSpecProfileGateway.ClassAssignmentRequest("wizard",1,0,"assign-runtime")).toCompletableFuture().join();
    ...
    private static void staleSessionFencesRuntime(){
        ControlledStore store=new ControlledStore(ClassProfile.empty(PLAYER,0));ProfileSessionRegistry sessions=new ProfileSessionRegistry();UUID old=sessions.begin(PLAYER);sessions.markReady(PLAYER,old);AtomicInteger runtimeCalls=new AtomicInteger();ClassSpecRuntimePort runtime=new ClassSpecRuntimePort(){public CompletionStage<Void> profileCommitted(UUID p,UUID t,ClassProfile a,ClassProfile b,MutationKind k){runtimeCalls.incrementAndGet();return CompletableFuture.completedFuture(null);}public CompletionStage<Void> failClosed(UUID p,UUID t,String r){return CompletableFuture.completedFuture(null);}};DefaultClassSpecProfileGateway gateway=new DefaultClassSpecProfileGateway(store,runtime,sessions);
    ...
    private static Harness harness(ClassProfile profile,ClassSpecRuntimePort runtime){FakeStore store=new FakeStore(profile);ProfileSessionRegistry sessions=readySessions();return new Harness(store,sessions,new DefaultClassSpecProfileGateway(store,runtime,sessions));}
    ...
    private static ClassProfile classOnly(String clazz){return ClassProfile.builder(PLAYER).primaryClassId(clazz).classLevel(1).build();}
    private static ClassProfile active(String spec,Map<String,String> mechanics){String clazz=ClassSpecCatalog.parentOf(spec);return ClassProfile.builder(PLAYER).primaryClassId(clazz).classLevel(25).activeSlot(LoadoutSlot.FIRST).loadout(LoadoutSlot.FIRST,new ClassLoadout(spec,LoadoutStatus.ACTIVE,null,Map.of(),MasteryProgress.empty(),null,Set.of(),"",CapstoneStatus.LOCKED,Map.of(),mechanics,"")).build();}
    ...
    private static class FakeStore implements ClassProfileMutationStore{volatile ClassProfile profile;volatile String blockReason="";int saves;FakeStore(ClassProfile p){profile=p;}public Optional<ClassProfile> cached(UUID id){return Optional.ofNullable(profile);}public Optional<String> sessionBlockReason(UUID id){return blockReason.isBlank()?Optional.empty():Optional.of(blockReason);}public CompletionStage<SaveResult> save(UUID id,long expected,ClassProfile candidate){saves++;if(profile==null||profile.revision()!=expected)return CompletableFuture.completedFuture(SaveResult.conflict(profile,profile==null?-1:profile.revision()));profile=candidate;return CompletableFuture.completedFuture(SaveResult.committed(candidate));}public CompletionStage<ClassProfile> recover(UUID id,String e,String a){return CompletableFuture.failedFuture(new UnsupportedOperationException());}public void blockSession(UUID id,String reason){blockReason=reason;}}
    private static final class ControlledStore extends FakeStore{CompletableFuture<SaveResult> pending;ClassProfile candidate;long expected;int saveCalls;ControlledStore(ClassProfile p){super(p);}@Override public CompletionStage<SaveResult> save(UUID id,long e,ClassProfile c){saveCalls++;expected=e;candidate=c;pending=new CompletableFuture<>();return pending;}void commitPending(){CompletableFuture<SaveResult> future=pending;ClassProfile next=candidate;long expectedRevision=expected;check(future!=null,"pending save");pending=null;candidate=null;if(profile.revision()!=expectedRevision)future.complete(SaveResult.conflict(profile,profile.revision()));else{profile=next;future.complete(SaveResult.committed(next));}}void awaitSaveCalls(int count){long deadline=System.nanoTime()+TimeUnit.SECONDS.toNanos(2);while(saveCalls<count&&System.nanoTime()<deadline)Thread.onSpinWait();check(saveCalls>=count,"timed out awaiting save call");}}
