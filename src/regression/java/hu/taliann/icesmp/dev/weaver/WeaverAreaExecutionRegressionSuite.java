...
public final class WeaverAreaExecutionRegressionSuite {
    ...
    public static void main(final String[] args) throws Exception {
        collection(); blockCap(); boundedTasks(); childExecution(false, false); childExecution(true, false); childExecution(true, true); invalidation(); membership(); pendingRecovery(); areaUndo();
    ...
    static final class RecoveryProvider extends WeaverContractRegressionSuite.FixtureProvider {
        ...
        @Override public RecoveryAssessment assessAreaRecovery(final RecoveryContext context, final SubjectSnapshot snapshot, final WeaverAreaCollection collection, final WeaverOperationRecord operation) {
            ...
            return new RecoveryAssessment(snapshot.revisionFingerprint().equals(operation.beforeFingerprint()) ? ObservedOperationState.BEFORE : ObservedOperationState.PARTIAL_OR_CONFLICT, false, Optional.empty(), "fixture");
        }
    }
    private static void pendingRecovery() throws Exception {
        ...
        final long preparedAt = System.currentTimeMillis();
        final var record = new WeaverOperationRecord(UUID.randomUUID(), HiddenDevAuthority.PRIMARY_DEVELOPER, "fixture", new ActionRequest(descriptor.id(), Map.of(), Lifetime.ONE_SHOT, IntegrityMode.SANDBOX),
                area, collection.fingerprint(), Optional.empty(), new OperationRecoveryPayload(1, Map.of(WeaverAreaRecoveryEvidence.KEY, WeaverAreaRecoveryEvidence.of(collection).encode())), OperationStatus.PREPARED, 0, preparedAt, preparedAt, Optional.empty(), false);
    ...
    private static void areaUndo() throws Exception {
        final Router router = new Router(); final Access access = new Access(router); final var engine = new WeaverAreaEngine(router, access);
        final var area = new AreaRef(UUID.randomUUID(), new CuboidArea(new AreaBounds(0, 64, 0, 0, 64, 0))); final var ref = new EntityRef(UUID.randomUUID());
        access.chunks.put(WeaverAreaEngine.chunks(area).getFirst(), new WeaverAreaAccess.ChunkSelection(List.of(ref), Optional.empty())); access.snapshots.put(ref, entity(area.worldId(), ref, 0, "after"));
        final var action = descriptor(AreaSupport.ENTITY_FANOUT, RiskLevel.MUTATING, 128, 0, 9);
        final var inverse = new ActionDescriptor("fixture.inverse", action.facetId(), action.label(), action.risk(), action.lifetimes(), action.integrityModes(), action.integrityImpacts(), action.subjects(), action.parameters(), action.areaSupport(), action.areaLimits(), false, Optional.empty(), 1);
        ...
        final var provider = new WeaverContractRegressionSuite.FixtureProvider("fixture", action, CoverageLevel.FULL_PROVIDER, Map.of(action.id(), "fixture.assess")) {
            @Override public Set<WeaverSubjectKind> supportedKinds() { return Set.of(WeaverSubjectKind.AREA); }
            @Override public ProviderCoverage coverage() { return new ProviderCoverage("fixture.domain", CoverageLevel.FULL_PROVIDER, "Isolated inverse fixture", Set.of(action.id(), inverse.id(), "fixture.state")); }
            ...
            @Override public PreparedAction prepareAreaUndo(final ProviderContext context, final SubjectSnapshot snapshot, final WeaverReceipt receipt, final WeaverAreaCollection collection) {
                calls.incrementAndGet(); check(collection.targets().size() == 1, "AREA inverse lacked current targets");
                final var stage = new ExecutionStage("fixture.inverse_child", SubjectRoute.owner(ref), Map.of(), (execution, payload) -> {
                    revision(access, ref, "restored"); return CompletableFuture.completedFuture(new StageResult("restored", Map.of(), Map.of()));
                }, Optional.of((compensation, payload) -> { revision(access, ref, "after"); return CompletableFuture.completedFuture(new StageResult("after", Map.of(), Map.of())); }), 5000);
                return new PreparedAction(UUID.randomUUID(), inverse, area, snapshot.revisionFingerprint(), List.of(stage), new OperationRecoveryPayload(1, Map.of()),
                        (plan, results, now) -> new WeaverReceipt(UUID.randomUUID(), plan.operationId(), "fixture", inverse.id(), area, RiskLevel.MUTATING, Lifetime.ONE_SHOT, IntegrityMode.SANDBOX, plan.expectedBeforeFingerprint(), results.getLast().afterFingerprint(), Map.of(), Map.of(), Optional.empty(), now, ReceiptStatus.COMMITTED));
            }
        };
        final var providers = WeaverContractRegressionSuite.registry(provider); providers.freezeAndValidate(); final var types = WeaverProjectionRegressionSuite.types();
        final var journal = new WeaverJournal(new Storage()); await(journal.load()); final var authority = authority(); final var collection = await(engine.collect(authority, area, inverse));
        final var snapshot = collection.decorate(new SubjectSnapshot(area, 1, "area", Map.of()));
        final var original = new WeaverOperationRecord(UUID.randomUUID(), authority.actor(), "fixture", new ActionRequest(action.id(), Map.of(), Lifetime.ONE_SHOT, IntegrityMode.SANDBOX), area,
                "original", Optional.empty(), new OperationRecoveryPayload(1, Map.of()), OperationStatus.PREPARED, 0, 1, 1, Optional.empty(), false);
        final var receipt = new WeaverReceipt(UUID.randomUUID(), original.operationId(), "fixture", action.id(), area, RiskLevel.MUTATING, Lifetime.ONE_SHOT, IntegrityMode.SANDBOX,
                "original", snapshot.revisionFingerprint(), Map.of(), Map.of(), Optional.of(new UndoSpec(inverse.id(), snapshot.revisionFingerprint(), Map.of())), 2, ReceiptStatus.COMMITTED);
        await(journal.prepare(original)); final var applied = await(journal.applied(original.operationId(), 0, receipt, WeaverEffectCommit.none(), 2)); await(journal.finishAudit(applied.operationId(), applied.revision()));
        final var context = new ProviderContext(authority, types, Lifetime.ONE_SHOT, IntegrityMode.SANDBOX); final var undo = new WeaverUndoCoordinator(journal, providers, engine);
        final var target = undo.target(authority, receipt.receiptId(), snapshot); final var prepared = undo.prepare(context, target.claim(), snapshot, Optional.of(collection));
        final var executor = new WeaverDurableExecutionCoordinator(router, journal, types);
        await(executor.execute("fixture", context, snapshot, new ActionRequest(inverse.id(), Map.of(), Lifetime.ONE_SHOT, IntegrityMode.SANDBOX), prepared,
                new PreparedEffects(new WeaverEffectIntent(Set.of(WeaverInfluenceTarget.subject(ref))), (plan, results, inverseReceipt, sequence) -> WeaverEffectCommit.none()), Optional.of(target.claim()), () -> authority));
        check(calls.get() == 1 && journal.snapshot().receipts().get(receipt.receiptId()).status() == ReceiptStatus.UNDONE && access.snapshots.get(ref).revisionFingerprint().equals("restored"), "AREA inverse did not use normal durable Undo");
