...
/**
 * Dependency-free Profile v2 gateway regressions for the Paplovag rollout: the allowlist admits
 * paladin, the two-loadout limit holds for the three-spec class and doctrines stay slot-local.
 */
public final class PaladinProfileRegressionSuite {

    private static final UUID PLAYER = UUID.fromString("00000000-0000-0000-0000-000000000461");
    private static int assertions;

    private PaladinProfileRegressionSuite() {
    }

    public static void main(final String[] args) {
        paladinSecondSlotUnlocksAndSwitches();
        threeSpecClassStillHoldsOnlyTwoLoadouts();
        paladinDoctrineMasteryAndCapstoneStaySlotLocal();
        System.out.println("Paladin profile regression suite passed. assertions=" + assertions);
    }

    private static void paladinSecondSlotUnlocksAndSwitches() {
        final Harness h = harness(ClassSpecSection.builder()
                .primaryClassId("paladin").classLevel(1).build());
        final var xp = h.gateway.mutateClassExperience(PLAYER,
                new ClassSpecProfileGateway.ClassExperienceRequest(
                        ...
                        100_000, 100, 0, 28, "paladin-xp-28")).toCompletableFuture().join();
        check(xp.committed(), "class XP mutation commits for paladin");
        check(h.store.profile.secondSpecUnlocked(),
                "paladin second spec unlocks through the allowlist");

        check(h.gateway.select(PLAYER, new ClassSpecProfileGateway.SelectRequest(
                        "holy", LoadoutSlot.FIRST, openGates()))
                .toCompletableFuture().join().committed(), "paladin learns Szentlélek");
        check(h.gateway.select(PLAYER, new ClassSpecProfileGateway.SelectRequest(
                        "retribution", LoadoutSlot.SECOND, openGates()))
                .toCompletableFuture().join().committed(), "paladin learns Megtorló second");
        final var switched = h.gateway.switchLoadout(PLAYER,
                        new ClassSpecProfileGateway.SwitchRequest(LoadoutSlot.SECOND))
                .toCompletableFuture().join();
        check(switched.committed(), "paladin loadout switching is enabled");
        check(h.gateway.activeSpecId(PLAYER).orElseThrow().equals("retribution"),
                "switch activates the Megtorló loadout");
    }

    private static void threeSpecClassStillHoldsOnlyTwoLoadouts() {
        final Harness h = harness(ClassSpecSection.builder()
                .primaryClassId("paladin").classLevel(30).classExperience(100_000)
                .secondSpecUnlocked(true)
                .loadout(LoadoutSlot.FIRST, loadout("holy", LoadoutStatus.ACTIVE))
                .loadout(LoadoutSlot.SECOND, loadout("protection", LoadoutStatus.INACTIVE))
                .activeSlot(LoadoutSlot.FIRST)
                .build());
        final var third = h.gateway.select(PLAYER, new ClassSpecProfileGateway.SelectRequest(
                "retribution", LoadoutSlot.SECOND, openGates())).toCompletableFuture().join();
        check(third.status() == ProfileMutationResult.Status.REJECTED,
                "a third spec cannot silently displace an occupied loadout");
        check(h.store.profile.loadout(LoadoutSlot.SECOND).specializationId().equals("protection"),
                "the occupied loadout is untouched — respec is the only path to the third spec");
    }

    private static void paladinDoctrineMasteryAndCapstoneStaySlotLocal() {
        final ClassLoadout elemental = loadout("holy", LoadoutStatus.ACTIVE)
                .withDoctrineChoice("level_30", "gyors_aldas")
                .withDoctrineChoice("level_40", "aldott_kez")
                .withMastery(new MasteryProgress(3, 310))
                .withCapstoneStatus(CapstoneStatus.COMPLETED);
        final ClassLoadout tidal = loadout("retribution", LoadoutStatus.INACTIVE)
                .withDoctrineChoice("level_30", "gyors_itelet")
        ...
        final ClassSpecSection profile = ClassSpecSection.builder()
                .revision(3).primaryClassId("paladin").classLevel(50).classExperience(999_999)
                .secondSpecUnlocked(true)
        ...
        check(profile.loadout(LoadoutSlot.FIRST).doctrineChoices()
                        .equals(Map.of("level_30", "gyors_aldas", "level_40", "aldott_kez")),
                "Szentlélek doctrines stay in their own slot");
        check(profile.loadout(LoadoutSlot.SECOND).doctrineChoices()
                        .equals(Map.of("level_30", "gyors_itelet")),
                "Megtorló doctrines stay in their own slot");
        ...
        expectFailure(() -> profile.loadout(LoadoutSlot.FIRST)
                        .withDoctrineChoice("level_30", "fenymeleg"),
