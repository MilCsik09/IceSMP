...
/**
 * Dependency-free Profile v2 gateway regressions for the Pap rollout: the allowlist admits
 * priest, the DARK Csontpap still answers to the existing gate system, and doctrines stay
 * slot-local.
 */
public final class PriestProfileRegressionSuite {

    private static final UUID PLAYER = UUID.fromString("00000000-0000-0000-0000-000000000491");
    private static int assertions;

    private PriestProfileRegressionSuite() {
    }

    public static void main(final String[] args) {
        priestSecondSlotUnlocksAndSwitches();
        darkBonePriestObeysTheExistingGates();
        priestDoctrineMasteryAndCapstoneStaySlotLocal();
        System.out.println("Priest profile regression suite passed. assertions=" + assertions);
    }

    private static void priestSecondSlotUnlocksAndSwitches() {
        final Harness h = harness(ClassSpecSection.builder()
                .primaryClassId("priest").classLevel(1).build());
        final var xp = h.gateway.mutateClassExperience(PLAYER,
                new ClassSpecProfileGateway.ClassExperienceRequest(
                        ...
                        100_000, 100, 0, 28, "priest-xp-28")).toCompletableFuture().join();
        check(xp.committed(), "class XP mutation commits for priest");
        check(h.store.profile.secondSpecUnlocked(),
                "priest second spec unlocks through the allowlist");

        check(h.gateway.select(PLAYER, new ClassSpecProfileGateway.SelectRequest(
                        "discipline", LoadoutSlot.FIRST, openGates()))
                .toCompletableFuture().join().committed(), "priest learns Fegyelem");
        check(h.gateway.select(PLAYER, new ClassSpecProfileGateway.SelectRequest(
                        "shadow", LoadoutSlot.SECOND, openGates()))
                .toCompletableFuture().join().committed(), "priest learns Árnyék second");
        final var switched = h.gateway.switchLoadout(PLAYER,
                        new ClassSpecProfileGateway.SwitchRequest(LoadoutSlot.SECOND))
                .toCompletableFuture().join();
        check(switched.committed(), "priest loadout switching is enabled");
        check(h.gateway.activeSpecId(PLAYER).orElseThrow().equals("shadow"),
                "switch activates the Árnyék loadout");
    }

    private static void darkBonePriestObeysTheExistingGates() {
        final Harness h = harness(ClassSpecSection.builder()
                .primaryClassId("priest").classLevel(30).classExperience(100_000)
                .secondSpecUnlocked(true).build());
        final var blocked = h.gateway.select(PLAYER, new ClassSpecProfileGateway.SelectRequest(
                "bone_priest", LoadoutSlot.FIRST, closedGates())).toCompletableFuture().join();
        check(blocked.status() == ProfileMutationResult.Status.REJECTED,
                "the DARK Csontpap is refused while its gates are unsatisfied");
        ...
        check(h.gateway.select(PLAYER, new ClassSpecProfileGateway.SelectRequest(
                        "bone_priest", LoadoutSlot.FIRST, satisfiedGates()))
                .toCompletableFuture().join().committed(),
                "satisfied gates admit the Csontpap through the existing DARK system");
        check(h.store.profile.loadout(LoadoutSlot.FIRST).specializationId().equals("bone_priest"),
                "the admitted DARK spec occupies its loadout");
    }

    private static void priestDoctrineMasteryAndCapstoneStaySlotLocal() {
        final ClassLoadout discipline = loadout("discipline", LoadoutStatus.ACTIVE)
                .withDoctrineChoice("level_30", "szeles_pajzs")
                .withDoctrineChoice("level_40", "tarto_vezekles")
                .withMastery(new MasteryProgress(5, 530))
                .withCapstoneStatus(CapstoneStatus.COMPLETED);
        final ClassLoadout shadow = loadout("shadow", LoadoutStatus.INACTIVE)
                .withDoctrineChoice("level_30", "mely_arnyek")
        ...
        final ClassSpecSection profile = ClassSpecSection.builder()
                .revision(5).primaryClassId("priest").classLevel(50).classExperience(999_999)
                .secondSpecUnlocked(true)
                .loadout(LoadoutSlot.FIRST, discipline)
                .loadout(LoadoutSlot.SECOND, shadow)
                .activeSlot(LoadoutSlot.FIRST)
                .build();
        check(profile.loadout(LoadoutSlot.FIRST).doctrineChoices()
                        .equals(Map.of("level_30", "szeles_pajzs", "level_40", "tarto_vezekles")),
                "Fegyelem doctrines stay in their own slot");
        check(profile.loadout(LoadoutSlot.SECOND).doctrineChoices()
                        .equals(Map.of("level_30", "mely_arnyek")),
                "Árnyék doctrines stay in their own slot");
        ...
        expectFailure(() -> profile.loadout(LoadoutSlot.FIRST)
                        .withDoctrineChoice("level_30", "korai_kegyelem"),
