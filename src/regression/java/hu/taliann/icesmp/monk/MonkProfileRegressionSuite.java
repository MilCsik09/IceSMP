...
/**
 * Dependency-free Profile v2 gateway regressions for the Szerzetes rollout: the allowlist admits
 * monk, the two-loadout limit holds for the three-spec class and doctrines stay slot-local.
 */
public final class MonkProfileRegressionSuite {

    private static final UUID PLAYER = UUID.fromString("00000000-0000-0000-0000-000000000451");
    private static int assertions;

    private MonkProfileRegressionSuite() {
    }

    public static void main(final String[] args) {
        monkSecondSlotUnlocksAndSwitches();
        threeSpecClassStillHoldsOnlyTwoLoadouts();
        monkDoctrineMasteryAndCapstoneStaySlotLocal();
        System.out.println("Monk profile regression suite passed. assertions=" + assertions);
    }

    private static void monkSecondSlotUnlocksAndSwitches() {
        final Harness h = harness(ClassSpecSection.builder()
                .primaryClassId("monk").classLevel(1).build());
        final var xp = h.gateway.mutateClassExperience(PLAYER,
                new ClassSpecProfileGateway.ClassExperienceRequest(
                        ...
                        100_000, 100, 0, 28, "monk-xp-28")).toCompletableFuture().join();
        check(xp.committed(), "class XP mutation commits for monk");
        check(h.store.profile.secondSpecUnlocked(),
                "monk second spec unlocks through the allowlist");

        check(h.gateway.select(PLAYER, new ClassSpecProfileGateway.SelectRequest(
                        "windwalker", LoadoutSlot.FIRST, openGates()))
                .toCompletableFuture().join().committed(), "monk learns Szélfutó");
        check(h.gateway.select(PLAYER, new ClassSpecProfileGateway.SelectRequest(
                        "mistweaver", LoadoutSlot.SECOND, openGates()))
                .toCompletableFuture().join().committed(), "monk learns Ködszövő second");
        final var switched = h.gateway.switchLoadout(PLAYER,
                        new ClassSpecProfileGateway.SwitchRequest(LoadoutSlot.SECOND))
                .toCompletableFuture().join();
        check(switched.committed(), "monk loadout switching is enabled");
        check(h.gateway.activeSpecId(PLAYER).orElseThrow().equals("mistweaver"),
                "switch activates the Ködszövő loadout");
    }

    private static void threeSpecClassStillHoldsOnlyTwoLoadouts() {
        final Harness h = harness(ClassSpecSection.builder()
                .primaryClassId("monk").classLevel(30).classExperience(100_000)
                .secondSpecUnlocked(true)
                .loadout(LoadoutSlot.FIRST, loadout("windwalker", LoadoutStatus.ACTIVE))
                .loadout(LoadoutSlot.SECOND, loadout("brewmaster", LoadoutStatus.INACTIVE))
        ...
        check(h.store.profile.loadout(LoadoutSlot.SECOND).specializationId().equals("brewmaster"),
                "the occupied loadout is untouched — respec is the only path to the third spec");
    }

    private static void monkDoctrineMasteryAndCapstoneStaySlotLocal() {
        final ClassLoadout elemental = loadout("windwalker", LoadoutStatus.ACTIVE)
                .withDoctrineChoice("level_30", "parducsap")
                .withDoctrineChoice("level_40", "vihar_okle")
                .withMastery(new MasteryProgress(3, 310))
                .withCapstoneStatus(CapstoneStatus.COMPLETED);
        final ClassLoadout tidal = loadout("mistweaver", LoadoutStatus.INACTIVE)
                .withDoctrineChoice("level_30", "friss_kod")
        ...
        final ClassSpecSection profile = ClassSpecSection.builder()
                .revision(3).primaryClassId("monk").classLevel(50).classExperience(999_999)
                .secondSpecUnlocked(true)
        ...
        check(profile.loadout(LoadoutSlot.FIRST).doctrineChoices()
                        .equals(Map.of("level_30", "parducsap", "level_40", "vihar_okle")),
                "Szélfutó doctrines stay in their own slot");
        check(profile.loadout(LoadoutSlot.SECOND).doctrineChoices()
                        .equals(Map.of("level_30", "friss_kod")),
                "Ködszövő doctrines stay in their own slot");
        ...
        expectFailure(() -> profile.loadout(LoadoutSlot.FIRST)
                        .withDoctrineChoice("level_30", "konnyed_lepes"),
