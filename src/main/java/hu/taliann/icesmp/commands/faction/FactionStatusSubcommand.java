...
public final class FactionStatusSubcommand implements FactionSubcommand {
    ...
    public boolean execute(final CommandSender sender, final String[] args) {
        ...
        try {
            ...
            whisperManager.getStage(player); // Validate the stage instead of reporting a fabricated CLEAN status.
        } catch (final RuntimeException unavailable) {
            player.sendMessage(messageManager.get("messages.faction-status-unavailable", "&cA frakcióprofil most nem olvasható. Próbáld újra."));
            return true;
        }
    ...
    private boolean swear(final Player player) {
        ...
        sinManager.sealDarkPact(player).whenComplete((sworn, failure) -> player.getScheduler().run(
                org.bukkit.plugin.java.JavaPlugin.getProvidingPlugin(FactionStatusSubcommand.class), task -> {
                    if (failure != null || !Boolean.TRUE.equals(sworn)) {
                        player.sendMessage(messageManager.get("messages.faction-oath-failed", "&cAz eskü mentése nem sikerült. Próbáld újra."));
                    } else player.sendMessage(messageManager.get("messages.faction-oath-sworn",
