...
public record RelicSection(Map<String,Map<String,String>> owned, Set<String> claimedMilestones, Map<String,Object> extensions) implements PlayerProfileSection {
    public RelicSection{owned=nestedStringMap(owned,512);claimedMilestones=ImmutableValues.stringSet(claimedMilestones,512);extensions=ImmutableValues.map(extensions);}
    @Override public ProfileSectionId sectionId(){return ProfileSectionId.RELICS;}
    public static RelicSection empty(long now){return new RelicSection(Map.of(),Set.of(),Map.of());}
