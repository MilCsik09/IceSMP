...
public record CompanionSection(Map<String,Map<String,Object>> roster, String activeCompanionId, Map<String,Object> extensions) implements PlayerProfileSection {
    public CompanionSection{roster=nestedObjectMap(roster,256);activeCompanionId=ImmutableValues.text(activeCompanionId,128);extensions=ImmutableValues.map(extensions);}
    @Override public ProfileSectionId sectionId(){return ProfileSectionId.COMPANIONS;}
    public static CompanionSection empty(long now){return new CompanionSection(Map.of(),"",Map.of());}
