...
public final class YamlPlayerProfileRepository implements PlayerProfileRepository {
    ...
    private static String operationWalName(String operationId){try{return "transaction-"+HexFormat.of().formatHex(java.security.MessageDigest.getInstance("SHA-256").digest(operationId.getBytes(StandardCharsets.UTF_8)));}catch(java.security.NoSuchAlgorithmException impossible){throw new IllegalStateException("SHA-256 unavailable",impossible);}}
