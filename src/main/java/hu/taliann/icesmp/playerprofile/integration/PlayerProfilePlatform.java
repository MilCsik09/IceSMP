...
public final class PlayerProfilePlatform {
 ...
 public synchronized CompletionStage<PlayerProfileRepository.ShutdownResult> shutdown(Duration timeout){
  Objects.requireNonNull(timeout);if(timeout.isNegative())throw new IllegalArgumentException("negative shutdown timeout");
  final long budgetNanos=Math.max(1L,timeout.toNanos());final long startedAt=System.nanoTime();
  started=false;
  ...
  final long httpBudget=Math.max(1L,budgetNanos/3L);
  CompletionStage<Void> httpStop=currentHttp==null?CompletableFuture.completedFuture(null):currentHttp.stop(Duration.ofNanos(httpBudget));
  return httpStop.handle((v,x)->null).toCompletableFuture()
   .completeOnTimeout(null,httpBudget,TimeUnit.NANOSECONDS)
   .thenCompose(v->{long elapsed=Math.max(0L,System.nanoTime()-startedAt);long remaining=Math.max(1L,budgetNanos-elapsed);return service.shutdown(Duration.ofNanos(remaining));})
   .toCompletableFuture().completeOnTimeout(new PlayerProfileRepository.ShutdownResult(false,0,"platform shutdown deadline exceeded"),budgetNanos,TimeUnit.NANOSECONDS);
 }
