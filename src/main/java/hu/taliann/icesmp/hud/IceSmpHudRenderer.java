...
public final class IceSmpHudRenderer {
    ...
    public Component render(final IceSmpHudModel model, final HudLayoutSnapshot layout) {
        ...
        output.append(glyph(-254, PANEL_FONT, themeGlyph(model.factionTheme()), 241, null, safeLayout));
        output.append(glyph(-254, WALLET_PANEL_FONT, '\uE105', 241, null, safeLayout));
        output.append(glyph(-254, DETAIL_PANEL_FONT, '\uE106', 241, null, safeLayout));
        output.append(glyph(-236, CLASS_FONT, classGlyph(model.classHud().classId()), 37, null, safeLayout));
        output.append(glyph(-88, UTILITY_FONT, '\uE132', 16, null, safeLayout));
        output.append(glyph(-234, UTILITY_FONT, '\uE131', 16, null, safeLayout));
        ...
        output.append(text(-190, HEADER_FONT, model.className() + spec, accent, 25, safeLayout));
        output.append(text(-190, SUBHEADER_FONT, model.faction(), color("A9B7C6", 0xA9B7C6), 22, safeLayout));
    ...
    private void drawMechanics(final TextComponent.Builder output, final IceSmpHudModel model,
        ...
        if ("death_knight".equals(model.classHud().classId())) {
            drawMetricIcon(output, model.classHud().classId(), model.classHud().metric(0), -234, layout);
            output.append(text(-217, MECHANIC_FONT, model.classHud().mechanicPrimary(), accent, 22, layout));
            int index = 0;
            for (final ClassHudSlot slot : model.classHud().slots()) {
                if (index >= 8) break;
                output.append(glyph(-234 + index * 27, RUNE_FONT, runeGlyph(slot), 19, null, layout));
                index++;
            }
        ...
        drawMetricIcon(output, model.classHud().classId(), primary, -234, layout);
        drawMetricIcon(output, model.classHud().classId(), secondary, -113, layout);
        output.append(text(-217, MECHANIC_FONT, model.classHud().mechanicPrimary(), accent, 20, layout));
        ...
        if (primary != null && primary.maximum() > 0.0D) {
            drawSegments(output, -234, METRIC_FONT, primary.percent(), SEGMENT_FILL, layout);
        }
        if (secondary != null && secondary.maximum() > 0.0D) {
            drawSegments(output, -113, METRIC_FONT, secondary.percent(), SEGMENT_GOLD, layout);
        }
    ...
    private static void drawCharges(final TextComponent.Builder output, final String classId,
        ...
        for (final ClassHudSlot slot : slots) {
            ...
            if (mechanic == null) {
                ...
                output.append(glyph(-234 + index * 12, CHARGE_FONT, fallback, 11, null, layout));
            } else {
                output.append(glyph(-234 + index * 12, MECHANIC_SLOT_FONT, mechanic, 11, null, layout));
            }
            index++;
        }
    }
    ...
    private static void drawMetricIcon(final TextComponent.Builder output, final String classId,
        ...
        if (glyph != null) output.append(glyph(x, MECHANIC_ICON_FONT, glyph, 15, null, layout));
    }
    ...
    private static void drawCurrencies(final TextComponent.Builder output, final IceSmpHudModel model,
        ...
        for (final hu.taliann.icesmp.managers.HudManager.HudCurrency currency : model.currencies()) {
            ...
            output.append(glyph(x, CURRENCY_FONT, currencyGlyph(currency.id()), 16, null, layout));
    ...
    private static void drawSegments(final TextComponent.Builder output, final int x, final Key font,
        ...
        for (int index = 0; index < SEGMENTS; index++) {
            output.append(glyph(x + index * 13, font, SEGMENT_TRACK, 13, null, layout));
            if (index < active) output.append(glyph(x + index * 13, font, fill, 13, null, layout));
        }
    }
    ...
    private static Component glyph(final int x, final Key font, final char glyph,
        ...
        final TextColor encoded = encodeLayoutColor(color == null ? TextColor.color(0xFFFFFF) : color, layout);
    ...
    private static Component text(final int x, final Key font, final String raw,
        ...
        return Component.text().append(space(anchoredX)).append(Component.text(value).font(font)
                        .color(encodeLayoutColor(color, layout)))
                .append(space(-anchoredX - width)).build();
    }
    ...
    static TextColor encodeLayoutColor(final TextColor desired, final HudLayoutSnapshot layout) {
        ...
        final int code = layout.shaderCode();
