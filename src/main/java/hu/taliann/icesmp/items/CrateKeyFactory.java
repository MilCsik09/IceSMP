...
public final class CrateKeyFactory {
    ...
    private ItemStack build(final String crateId, final Material material, final String keyName,
        ...
        final List<Component> rewardOdds = new ArrayList<>();
        final int shown = Math.min(3, odds.size());
        for (int index = 0; index < shown; index++) {
            final CrateManager.RewardOdds reward = odds.get(index);
            rewardOdds.add(SERIALIZER.deserialize(TextUtil.color("&8◆ &7" + reward.description()
                            + " &8— &e" + CrateFormatting.decimal(reward.percent()) + "%"))
                    .decoration(TextDecoration.ITALIC, false));
        }
        if (odds.size() > shown) {
            rewardOdds.add(SERIALIZER.deserialize(TextUtil.color(
                            "&8…és további " + (odds.size() - shown) + " jutalom"))
                    .decoration(TextDecoration.ITALIC, false));
        }
        final String plainCrateName = crateName == null ? ""
                : crateName.replaceAll("[&§][0-9a-fk-orA-FK-OR]", "");
        meta.lore(ItemTooltipProfiles.crateKey(plainCrateName, rewardOdds));
        ...
        ItemDataFactory.applyTooltipStyle(itemStack,
                ItemTooltipProfiles.styleId(ItemTooltipProfiles.Profile.KEY));
        return itemStack;
    }
