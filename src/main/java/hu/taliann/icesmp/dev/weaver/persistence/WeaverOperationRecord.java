...
public record WeaverOperationRecord(UUID operationId, UUID actorId, String providerId, ActionRequest request,
                                    ...
                                    long preparedAt, long updatedAt, Optional<WeaverReceipt> receipt, boolean pendingAudit, Optional<WeaverUndoClaim> undoClaim) {
    public WeaverOperationRecord(final UUID operationId, final UUID actorId, final String providerId, final ActionRequest request,
            final SubjectRef subject, final String beforeFingerprint, final Optional<String> afterFingerprint,
            final OperationRecoveryPayload recoveryPayload, final OperationStatus status, final long revision,
            final long preparedAt, final long updatedAt, final Optional<WeaverReceipt> receipt, final boolean pendingAudit) {
        this(operationId, actorId, providerId, request, subject, beforeFingerprint, afterFingerprint, recoveryPayload, status, revision, preparedAt, updatedAt, receipt, pendingAudit, Optional.empty());
    }
    public WeaverOperationRecord {
        ...
        if (undoClaim.isPresent() && !undoClaim.get().expectedFingerprint().equals(beforeFingerprint)) throw new IllegalArgumentException("Undo claim fingerprint differs from operation");
