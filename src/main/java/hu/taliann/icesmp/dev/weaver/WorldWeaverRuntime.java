...
public final class WorldWeaverRuntime {
    ...
    public WorldWeaverRuntime(final JavaPlugin plugin, final DevItemManager artifacts, final ItemIdentityService identity,
        ...
        kernel = new WorldWeaverKernel(artifacts, providers, types, snapshots, slots,
                new WorldWeaverGUI(), new WeaverExecutionCoordinator(router, types, journal, () -> started && !closed), new hu.taliann.icesmp.dev.weaver.execution.WeaverUndoCoordinator(journal, providers, areas), areas, () -> started && !closed && journal.ready());
    ...
    private void maintainEffects() {
        if (!closed && journal.ready() && maintaining.compareAndSet(false, true)) {
            journal.expireProjections(System.currentTimeMillis(), false)
                    ...
                    .thenCompose(ignored -> closed ? java.util.concurrent.CompletableFuture.completedFuture(null) : recovery.nativeProjectionsAvailable())
    ...
    public void shutdown() {
        ...
        providers.clearSessions(); kernel.shutdown(); recovery.close(); influenceDispatcher.close(); router.close();
