...
public final class WeaverAreaEngine implements AutoCloseable {
    ...
    public PreparedAction guard(final WeaverAreaCollection collection, final PreparedAction prepared) {
        ...
        for (int i = 0; i < collection.targets().size(); i++) {
            ...
            if (!stage.owner().equals(SubjectRoute.owner(before.ref())) || prepared.descriptor().risk().ordinal() >= RiskLevel.MUTATING.ordinal() && stage.compensate().isEmpty()) throw new IllegalArgumentException("AREA child ownership or compensation missing");
