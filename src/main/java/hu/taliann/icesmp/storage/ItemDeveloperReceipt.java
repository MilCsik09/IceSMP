...
public record ItemDeveloperReceipt(ItemMutationJournal.Entry entry, Kind kind, int sourceSlot,
                                   ...
                                   long afterRevision, State state, long resolvedAt) {
    ...
    public ItemDeveloperReceipt {
        ...
        Objects.requireNonNull(resultItemId); Objects.requireNonNull(state);
        if (!HiddenDevAuthority.isDeveloper(entry.playerId()) || !entry.type().equals("DEV_" + kind.name())
        ...
        if (entry.beforeInventory().get(sourceSlot).equals("-") || entry.afterInventory().get(targetSlot).equals("-")) {
    ...
    public ItemDeveloperReceipt resolve(State next, long now) {
        ...
        return new ItemDeveloperReceipt(entry, kind, sourceSlot, targetSlot, resultItemId,
                beforeRevision, afterRevision, next, Math.max(entry.createdAt(), now));
    }
}
