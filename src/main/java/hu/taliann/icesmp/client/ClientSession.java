...
public final class ClientSession {
    ...
    /**
     * A teljes session-boríték kapuja: a negotiált protokoll és generation
     * ellenőrzése megelőzi a sequence publikálását.
     */
    public boolean acceptInbound(final int envelopeProtocol, final long envelopeGeneration,
                                 final long sequence, final long nowMillis) {
        if (envelopeProtocol != protocolVersion || envelopeGeneration != generation) {
            inboundDropped.incrementAndGet();
            return false;
        }
        return acceptInbound(sequence, nowMillis);
    }
