...
public final class CrateLedger {
    ...
    public Mutation record(final UUID playerId, final String name, final String crateId, final int opens,
        ...
        // Compute every failure-prone value before mutating the map. In particular, a cooldown
        // overflow must not leave a count/name change behind without a rollback token.
        ...
        state.lastKnownName = name == null ? previousName : name;
        state.counts.put(crateId, newCount);
        if (cooldownMillis > 0L) {
            state.cooldowns.put(crateId, newCooldown);
        }
        return new Mutation(playerId, crateId, previousCount, previousCooldown, previousName,
                newCount, newCooldown);
    }
    ...
    public long remainingCooldown(final UUID playerId, final String crateId, final long now) {
        ...
        return state == null ? 0L : Math.max(0L, state.cooldowns.getOrDefault(crateId, 0L) - now);
    }
    ...
    public record Mutation(UUID playerId, String crateId, long previousCount, long previousCooldown,
                           String previousName, long newCount, long newCooldown) {
    }
