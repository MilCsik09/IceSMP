...
public record ProfileMutationResult<P>(Status status,P durableProfile,String detail,boolean sessionBlocked){
    ...
    /** Durable authority changed even if the captured runtime generation could not finish. */
    public boolean durableMutationApplied(){return status==Status.COMMITTED||status==Status.RUNTIME_EFFECT_FAILED||status==Status.STALE_SESSION;}
    /** Idempotent replay/no-op is also a successful durable outcome for economic callers. */
    public boolean durableOutcomeAccepted(){return durableMutationApplied()||status==Status.NO_CHANGE;}
    /** Only these statuses permit callbacks tied to the captured live runtime generation. */
    public boolean runtimeGenerationUsable(){return status==Status.COMMITTED||status==Status.NO_CHANGE;}
    public Optional<P> durableProfileOptional(){return Optional.ofNullable(durableProfile);}
