...
public record ClassLoadout(String specializationId, LoadoutStatus status, SealReason sealReason,
    ...
    public ClassLoadout withStatus(LoadoutStatus s,SealReason r){return copy(s,r,doctrineChoices,companionRoster,mechanicState,mastery,capstoneStatus);}
    public ClassLoadout withCompanionRoster(Map<UUID,CompanionProfile> r){return copy(status,sealReason,doctrineChoices,r,mechanicState,mastery,capstoneStatus);}
    public ClassLoadout withMechanicState(Map<String,String> m){return copy(status,sealReason,doctrineChoices,companionRoster,m,mastery,capstoneStatus);}
    public ClassLoadout withMastery(MasteryProgress m){return copy(status,sealReason,doctrineChoices,companionRoster,mechanicState,Objects.requireNonNull(m),capstoneStatus);}
    public ClassLoadout withDoctrineChoice(String tier,String choice){final String t=ClassSpecCatalog.normalize(tier),c=ClassSpecCatalog.normalize(choice);if(t.isEmpty()||c.isEmpty())throw new IllegalArgumentException("Doctrine tier and choice must be non-blank");final Map<String,String> next=new LinkedHashMap<>(doctrineChoices);final String existing=next.putIfAbsent(t,c);if(existing!=null&&!existing.equals(c))throw new IllegalStateException("Doctrine tier is already committed: "+t);return existing!=null?this:copy(status,sealReason,next,companionRoster,mechanicState,mastery,capstoneStatus);}
    public ClassLoadout withCapstoneStatus(CapstoneStatus next){return copy(status,sealReason,doctrineChoices,companionRoster,mechanicState,mastery,Objects.requireNonNull(next));}
    public boolean isActivatable(){return status==LoadoutStatus.ACTIVE||status==LoadoutStatus.INACTIVE;}
    private ClassLoadout copy(LoadoutStatus s,SealReason reason,Map<String,String> doctrines,Map<UUID,CompanionProfile> roster,Map<String,String> mechanics,MasteryProgress progress,CapstoneStatus capstone){return new ClassLoadout(specializationId,s,reason,doctrines,progress,soulbond,favoriteSpells,selectedSpell,capstone,roster,mechanics,diagnosticNote);}
